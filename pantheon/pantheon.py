...
class Pantheon():
    ...
    def __init__(self, server, server_2, api_key, session, errorHandling = False, requestsLoggingFunction = None, debug=False):
        """
    ...
    async def fetch(self, url, method="GET", data=None):
        ...
        try:
            if method=="GET":
                ...
                response = await self._session.request(method, url, headers=headers, data=json.dumps(data))
        #In case of timeout
        except Exception as e:
            return None
